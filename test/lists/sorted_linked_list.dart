...
void main() {
  ...
  SortedLinkedList listWithOneElement;
  SortedLinkedList theStooges;
  SortedLinkedList lostNumbers;
  ...
  test('Create list with custom sorting criteria', () {
    var customList = SortedLinkedList(compareFn: (a, b) => a.compareTo(b) >= 0);
    customList.insert(1);
    customList.insert(2);
    customList.insert(3);
  ...
  test('Remove returns null if index is invalid', () {
    expect(lostNumbers.remove(1000), isNull);
    expect(lostNumbers.remove(-1), isNull);
    expect(emptyList.remove(0), isNull);
  });
  ...
  test('Trying to index with an invalid number returns null', () {
    expect(lostNumbers.at(1000), isNull);
    expect(lostNumbers.at(-1), isNull);
    expect(emptyList.at(0), isNull);
  });
